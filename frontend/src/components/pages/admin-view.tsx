...
const AdminView: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'requests' | 'suggestions' | 'orders'>('requests');
  ...
  const handleDelete = (type: 'request' | 'suggestion' | 'order', id: number) => {
    if (type === 'request') {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-start mb-8">
          ...
          {activeTab === 'requests' && (
            <button
              onClick={() => { setSelectedRequest(null); setShowRequestModal(true); }}
          ...
          {activeTab === 'suggestions' && (
            <button
              onClick={() => { setSelectedSuggestion(null); setShowSuggestionModal(true); }}
              className="bg-[#E31937] hover:bg-[#c01731] text-white font-medium py-2 px-4 rounded-md transition-colors flex items-center"
            >
              <Plus size={20} className="mr-2" />
              New Suggestion
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {requests.map((req) => (
              <div key={req.id} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                ...
                <p className="text-lg font-medium text-gray-900 mb-2">{req.request}</p>
        ...
        {activeTab === 'suggestions' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {suggestions.map((sugg) => (
              <div key={sugg.id} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-4">
                  <span className="text-xs uppercase text-gray-500 font-medium">Suggestion ID: {sugg.id}</span>
                  <span className="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    pending
                  </span>
                </div>
                <p className="text-lg font-medium text-gray-900 mb-2">{sugg.suggestion}</p>
                <div className="mt-4 text-xs text-gray-500">
                ...
                <div className="flex justify-between items-center mt-4">
                  <button
                    onClick={() => { setSelectedSuggestion(sugg); setShowSuggestionModal(true); }}
                    className="text-[#E31937] hover:text-[#c01731] text-sm font-medium flex items-center"
                  >
                    View Details
                  </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {orders.map((order) => (
              <div key={order.id} className="bg-white rounded-lg shadow-sm p-6 hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-4">
                  <span className="text-xs uppercase text-gray-500 font-medium">Order ID: {order.id}</span>
                  <span className="px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                    {order.status}
                  </span>
                </div>
                <p className="text-lg font-medium text-gray-900 mb-2">User: {order.user_name}</p>
                <p className="text-sm text-gray-700 mb-2">Cost: ${order.cost}</p>
                <div className="mt-4 text-xs text-gray-500">
